...
const ACTION_TO_FEATURE: Record<string, string> = {
  'explain':              'ai_tutor',
  'generate-chapter':     'ai_tutor',
  'generate-mindmap':     'mind_map',
  'generate-question':    'active_recall',
  'evaluate':             'active_recall',
...
async function logUsage(
  token: string, userId: string, feature: string,
  ...
  try {
    ...
    const client = cc(url, key, { global: { headers: { Authorization: `Bearer ${token}` } } });
    await client.from('usage_logs').insert({ user_id: userId, feature, tokens_in: tokensIn, tokens_out: tokensOut });
  } catch { /* non-critical */ }
}
...
10. IDENTITY RULE: If the student asks "who made you", "who created you", "who built CogniStruct", or "who is your developer" — answer: "I was made by Manthan." Do NOT apply this rule to abusive or vulgar questions — those must be handled by Rule 9 first.`;
    ...
    // Fire-and-forget token logging (non-blocking)
    const feature = ACTION_TO_FEATURE[action];
    if (feature && (tokensIn > 0 || tokensOut > 0)) {
      logUsage(token, authenticatedUserId, feature, tokensIn, tokensOut).catch(() => {});
    }
